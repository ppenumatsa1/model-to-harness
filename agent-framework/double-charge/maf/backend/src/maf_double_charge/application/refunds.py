...
from .errors import RefundIdempotencyConflictError, UncertainRefundResponseError
from .models import RefundLedgerEntry, WorkflowState
from .ports import Actions, Repository
...
class DurableRefundService:
    ...
    async def submit(
        ...
        try:
            refund = await asyncio.to_thread(actions.submit_refund, state.idempotency_key)
