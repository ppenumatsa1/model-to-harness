...
import { api } from "./api";
import type { CasePage, CaseSummary, RunView } from "./types";
...
export function useCaseHistory() {
    ...
    try {
      ...
      setItems((current) => {
        ...
        result.items.forEach((item) => {
          ...
          merged.set(item.case_id, changed ? snapshots.current.get(item.case_id) ?? item : item);
        });
      ...
      // A disjoint refreshed head has a gap. Walk its cursor until it overlaps a
      // previously loaded range, then reuse that range's older frontier.
  ...
  const update = useCallback((view: RunView, insert = false) => {
    const summary: CaseSummary = {
      ...
      created_at: view.created_at, updated_at: view.state.updated_at ?? view.created_at,
      status: view.state.status, current_step: view.state.current_step,
      approval_required: view.state.approval_required, terminal_status: view.state.terminal_status ?? null
    };
