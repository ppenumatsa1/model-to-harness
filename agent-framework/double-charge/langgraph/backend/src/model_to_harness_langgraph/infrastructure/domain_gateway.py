...
class SharedDomainGateway:
    ...
    def __init__(self) -> None:
        ...
        self._evidence_type = shared.DuplicateEvidence
    ...
    async def detect_duplicate(
        ...
        context = self._context(run_id, scenario_id)
        evidence = context.billing.detect_duplicate(context.fixture.scenario_input.account_id)
        return ToolResult(
            ok=True,
            value={
    ...
    async def validate_billing(
        ...
        del customer_id, charges
        context = self._context(run_id, scenario_id)
        evidence = self._evidence_type.model_validate(duplicate_evidence)
        validation = context.billing.validate_duplicate(evidence)
        return ToolResult(
            ok=validation.valid,
    ...
    async def validate_policy(
        ...
        context = self._context(run_id, scenario_id)
        evidence = self._evidence_type.model_validate(duplicate_evidence)
        assessment = context.policy.assess(evidence, context.fixture.charges)
    ...
    async def submit_refund(
        ...
        try:
            ...
            return ToolResult(
                ...
                safe_summary=("The refund was stored but the deterministic response was uncertain"),
            )
