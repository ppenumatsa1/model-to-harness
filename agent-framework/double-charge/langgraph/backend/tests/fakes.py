...
from model_to_harness_langgraph.domain_gateway import ToolResult
...
class FakeDomainGateway:
    ...
    async def detect_duplicate(
        ...
        duplicate = len(charges) == 2 and charges[0]["purchase_reference"] == charges[1][
            "purchase_reference"
        ]
        return ToolResult(
            ok=True,
    ...
    async def validate_billing(
        ...
        del run_id, customer_id, charges, scenario_id
        return ToolResult(
            ok=True,
            value={"captured": True},
    ...
    async def verify_refund(
        ...
        return ToolResult(
            ...
            value={
                ...
                "refund_id": durable_refund_id
                or self.refunds.get(idempotency_key, "rf-1"),
            },
