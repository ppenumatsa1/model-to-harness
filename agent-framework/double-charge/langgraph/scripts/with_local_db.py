"""Run an explicit command against this lane's loopback Compose database."""
...
def main() -> None:
    ...
    if (
        config["name"] != "double-charge-lg"
        or port["host_ip"] != "127.0.0.1"
        or str(port["published"]) != "25432"
        or port["target"] != 5432
    ):
        raise SystemExit("Refusing a database outside this lane's loopback Compose contract.")
    ...
    url = (
        ...
        f"{quote(values['POSTGRES_PASSWORD'], safe='')}@127.0.0.1:25432/"
    ...
    environment = {
        **os.environ,
        ...
        "LANGGRAPH_SCHEMA": "langgraph_app_local",
        "LANGGRAPH_CHECKPOINT_SCHEMA": "langgraph_checkpoints_local",
