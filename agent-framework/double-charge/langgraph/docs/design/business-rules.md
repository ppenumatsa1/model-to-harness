# LangGraph business scenarios and rules

Start here for the six scenarios in the LangGraph demo picker, their business
outcomes, and the approval rules. Refunds and notifications are simulated, not real
payment or delivery operations. See [user flow](userflow.md) for the UI journey and
...
`verification-mismatch` is not a normal demo choice. It remains available to
