...
export function AuditPanel({ events, run }: { events: NativeEvent[]; run?: Workspace }) {
  ...
  return <section className="panel audit-panel" aria-labelledby="audit-title">
    ...
    {run && <dl>
      ...
      <dt>Current resolution (snapshot)</dt><dd>{resolutionLabel(run.outcome?.terminal_status ?? run.state.terminal_status)}</dd>
    </dl>}
    <p className="boundary-note">Recorded business actions, oldest first. Technical records remain in the execution timeline.</p>
    {incompatible && <p role="status" className="boundary-note">Some historical actor information is unknown / not recorded. Missing facts are not inferred.</p>}
    <ol className="audit-events" aria-label="Business actions" tabIndex={0}>
      {entries.map((entry) => <li key={entry.id}>
        <strong>{entry.title}</strong><p>{entry.description}</p><p>{entry.actor}</p>
        <time dateTime={entry.recordedAt}>{new Date(entry.recordedAt).toLocaleString()}</time>
        {entry.evidence.length > 0 && <details><summary>Business evidence</summary>
          <dl>{entry.evidence.map(({ label, value }) => <div key={label}><dt>{label}</dt><dd>{value}</dd></div>)}</dl>
        </details>}
      </li>)}
    </ol>
