...
export function developmentSettings(
  ...
  const host = setting("CHECKOUT_COPILOT_API_HOST", "127.0.0.1");
  const portText = setting("CHECKOUT_COPILOT_API_PORT", "8030").trim();
  const port = Number(portText);
  if (!/^\d+$/.test(portText) || !Number.isInteger(port) || port < 1 || port > 65535) {
    throw new Error("CHECKOUT_COPILOT_API_PORT must be an integer between 1 and 65535.");
  }
  const hostname = host.length <= 253 && host.split(".").every(
    (label) => /^[a-z\d](?:[a-z\d-]{0,61}[a-z\d])?$/i.test(label)
  );
  if (!isIP(host) && !hostname) {
    throw new Error("CHECKOUT_COPILOT_API_HOST must be an IP address or hostname.");
  }
  const targetHost = host === "0.0.0.0" ? "127.0.0.1" : host === "::" ? "::1" : host;
  const apiTarget = `http://${isIP(targetHost) === 6 ? `[${targetHost}]` : targetHost}:${port}`;
  const originText = setting("CHECKOUT_COPILOT_FRONTEND_ORIGIN", "http://127.0.0.1:5180");
  let origin: URL;
  try {
    ...
    throw new Error("CHECKOUT_COPILOT_FRONTEND_ORIGIN must be a loopback HTTP origin without credentials or a path.");
  }
  return {
    apiTarget,
