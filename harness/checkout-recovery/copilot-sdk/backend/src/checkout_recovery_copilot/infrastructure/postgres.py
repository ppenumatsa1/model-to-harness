...
from checkout_recovery_copilot.application.models import AuditEvent, CaseRecord, RemediationIntent
...
class PostgresCaseRepository:
    ...
    def ready(self) -> bool:
        with self.connection() as connection:
            connection.execute("SELECT case_id FROM checkout_recovery_copilot_sessions LIMIT 0")
        return True

    def save_framework_state(self, case_id: str, state: dict[str, Any]) -> None:
        with self.connection() as connection:
            connection.execute(
                """INSERT INTO checkout_recovery_copilot_sessions (case_id, framework_state)
                   VALUES (%s, %s::jsonb)
    ...
    def framework_state_for(self, case_id: str) -> dict[str, Any] | None:
        with self.connection() as connection:
            row = connection.execute(
                "SELECT framework_state FROM checkout_recovery_copilot_sessions WHERE case_id = %s",
                (case_id,),
            ).fetchone()
        return row["framework_state"] if row is not None else None
