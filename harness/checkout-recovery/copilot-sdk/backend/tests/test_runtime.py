...
import pytest
from checkout_recovery_copilot import bootstrap
...
@pytest.mark.parametrize("host", ["api", "hosted"])
def test_failed_native_start_preserves_configured_harness_identity(monkeypatch, host):
    investigator = Mock()
    investigator.investigate.side_effect = InvestigationIncompleteError("private native failure")
    monkeypatch.setattr(bootstrap, "CopilotInvestigator", Mock(return_value=investigator))
    repository = Mock(wraps=InMemoryCaseRepository(), close=Mock())
    monkeypatch.setattr(bootstrap, "PostgresCaseRepository", Mock(return_value=repository))
    runtime = bootstrap.create_runtime(
        Settings(
            _env_file=None, database_url="configured", execution_mode="copilot",
    ...
    try:
        case = runtime.service.start_case("recoverable-inventory-reservation")
        assert case.harness_mode == "copilot"
        assert case.failure_code.value == "harness_failed"
        assert runtime.service.resume_case(case.case_id) == case
        assert "private native failure" not in (
            runtime.service.get_case_response(case.case_id).model_dump_json()
        )
    finally:
        runtime.close()
...
def test_development_defaults_to_scripted_without_foundry(monkeypatch):
    monkeypatch.delenv("CHECKOUT_COPILOT_EXECUTION_MODE", raising=False)
...
def test_explicit_maf_mode_requires_foundry_configuration():
    ...
    with pytest.raises(ValueError, match="Copilot execution requires"):
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("api", {"execution_mode": "copilot"}),
        ("hosted", {}),
        ("hosted", {"execution_mode": "copilot", "database_url": None}),
    ],
)
