...
def test_mapped_and_native_response_namespaces():
    response = json.dumps({"harness_mode": "copilot", "verification_result": None})
    expected = json.dumps({"harness_mode": "copilot", "verification_result": None})
...
@pytest.mark.parametrize(
    ...
    [
        ...
        {"harness_mode": "copilot", "verification_result": None, "prompt": "forbidden"},
        {"harness_mode": "copilot", "verification_result": False},
    ],
)
    ...
    assert (
        grade(
            {},
            {
                ...
                "expected_behavior": '{"harness_mode":"copilot","verification_result":null}',
            },
        )
        == 0
    )
...
def test_boolean_is_not_interchangeable_with_number():
    assert (
        grade(
            {},
            {
                "response": '{"harness_mode":"copilot","verification_result":0}',
...
def test_invalid_response_fails(response):
    assert grade({}, {"response": response, "expected_behavior": '{"harness_mode":"copilot"}'}) == 0
...
def test_invalid_or_empty_expectations_fail(expected):
    assert (
        grade(
            {},
            {"response": '{"harness_mode":"copilot"}', "expected_behavior": expected},
        )
        == 0
    )
...
def test_missing_response_fails():
    assert grade({}, {"expected_behavior": '{"harness_mode":"copilot"}'}) == 0
