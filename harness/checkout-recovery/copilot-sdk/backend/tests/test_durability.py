...
import pytest
from checkout_recovery_copilot.application import CheckoutRecoveryService, InvalidCaseCommandError
...
def test_native_state_is_private_and_business_resume_does_not_load_it(repository):
    service = CheckoutRecoveryService(repository)
    case = service.start_case("captured-payment-approved-remediation")
    state = {"session_id": "synthetic-session", "native_archive": {"file": "opaque-data"}}
    with repository.transaction(case.case_id):
        repository.save_framework_state(case.case_id, state)
    state["native_archive"]["file"] = "changed-after-save"
    loaded = repository.framework_state_for(case.case_id)
    assert loaded["native_archive"]["file"] == "opaque-data"
    loaded["native_archive"]["file"] = "changed-after-load"
    assert repository.framework_state_for(case.case_id)["native_archive"]["file"] == "opaque-data"
    assert repository.framework_state_for(str(uuid4())) is None
    assert "opaque-data" not in service.get_case_response(case.case_id).model_dump_json()

    class UnavailableInvestigator:
        def investigate(self, simulator):
            raise AssertionError("business resume must not invoke the model")

    resumed_service = CheckoutRecoveryService(repository, investigator=UnavailableInvestigator())
    resumed_service.record_approval(
        case.case_id,
        decision=CheckoutApprovalDecision.APPROVED,
    ...
    assert resumed_service.resume_case(case.case_id).verification_result
