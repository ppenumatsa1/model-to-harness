...
def release_source(args: argparse.Namespace) -> str:
    ...
    if manifest:
        ...
        return module.verify(manifest)
    require_source(args.source_commit or "")
    return args.source_commit


def require_environment(env: dict) -> None:
    if not re.fullmatch(r"(?:rg-)?crcopilot(?:-[a-z0-9]+)*", env.get("AZURE_RESOURCE_GROUP", "")):
        raise SystemExit("Release requires an isolated crcopilot resource group")
    if not env.get("AZURE_SUBSCRIPTION_ID"):
        raise SystemExit("Release requires an explicit subscription")
    project = env.get("AZURE_AI_PROJECT_ID")
    scope = (
        f"/subscriptions/{env['AZURE_SUBSCRIPTION_ID']}"
        f"/resourceGroups/{env['AZURE_RESOURCE_GROUP']}/providers/"
    )
    if project and not project.lower().startswith(scope.lower()):
        raise SystemExit("Foundry project is outside the isolated release scope")


def require_bootstrap(folder: Path) -> None:
    outputs = folder / "app-outputs.json"
    if outputs.exists() and json.loads(outputs.read_text()).get("frontendUrl", {}).get("value"):
        raise SystemExit("Existing apps require update-existing; foundation redeploy is forbidden")
...
def update_existing(args: argparse.Namespace, env: dict, folder: Path) -> None:
    ...
    require_release_dependencies(ROOT / "requirements-cloud.lock", installed=True)
    source = release_source(args)
    ...
    for kind, repository, image in (
        ("backend", "checkout-recovery-copilot-api", args.backend_image),
        ("frontend", "checkout-recovery-copilot-web", args.frontend_image),
    ):
        ...
        tags = [tag for tag in metadata.get("tags", []) if tag.startswith(source + "-")]
        if metadata["digest"] != image.split("@")[1] or len(tags) != 1:
            raise SystemExit(
                "Image lacks an unambiguous matching source-commit tag or manifest tag"
            )
        plans.append(
            {
                "name": name,
    ...
    if release_source(args) != source:
        raise SystemExit("Source identity changed during release")
    ...
    result = {"source_identity": source, "apps": revisions, "passed": True}
...
def main() -> None:
    ...
    sources.add_argument("--source-commit")
    sources.add_argument("--source-manifest", type=Path)
    ...
    if args.step in {"migrate", "e2e", "browser", "evidence"}:
        ...
        process_env = {
            **os.environ,
            "CHECKOUT_COPILOT_DATABASE_URL": env["CHECKOUT_COPILOT_DATABASE_URL"],
            "CHECKOUT_UI_USERNAME": values["uiUsername"],
            "CHECKOUT_UI_PASSWORD": values["uiPassword"],
            "CHECKOUT_COPILOT_BASE_URL": outputs["frontendUrl"]["value"],
        }
        if args.step == "migrate":
            command = [sys.executable, "scripts/migrate.py", "--apply"]
        elif args.step == "e2e":
            command = [
                sys.executable,
                "scripts/e2e.py",
    ...
    if args.step == "prepare":
        ...
        params = {
            ...
            "foundryModelDeployment": env["CHECKOUT_COPILOT_FOUNDRY_MODEL_DEPLOYMENT"],
            "foundryAccountName": env["AZURE_AI_ACCOUNT_NAME"],
            "foundryProjectName": env["AZURE_AI_PROJECT_NAME"],
            "maxAutoInventoryQuantity": int(
                env.get("CHECKOUT_COPILOT_MAX_AUTO_INVENTORY_QUANTITY", "1")
            ),
    ...
    if args.step == "configure":
        ...
        run(["azd", "env", "set", "CHECKOUT_COPILOT_DATABASE_URL", url, "-e", args.environment])
        print("Configured hosted PostgreSQL secret without printing its value")
        return
    if args.step in {"foundation", "apps"}:
        require_bootstrap(folder)
    ...
    if args.step == "apps":
        ...
        outputs = json.loads((folder / "app-outputs.json").read_text())
        registry = outputs["registryLoginServer"]["value"]
        image_reference(args.backend_image, registry, "checkout-recovery-copilot-api")
        image_reference(args.frontend_image, registry, "checkout-recovery-copilot-web")
