...
def main() -> None:
    ...
    parser.add_argument(
        ...
        help="Retain the owned session for telemetry checks; the caller must stop it afterward.",
    )
    ...
    try:
        ...
        for row in cases[:1] if args.smoke else cases:
            ...
            if result.get("harness_mode") != "copilot":
                raise HostedVerificationError("Real Copilot investigation is required")
        ...
        if args.keep_session:
            print(f"Owned Hosted session retained for telemetry: {session_id}")
        else:
