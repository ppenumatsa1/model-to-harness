...
def main() -> None:
    ...
    if (
        config["name"] != "checkout-recovery-copilot"
        or port["host_ip"] != "127.0.0.1"
        or str(port["published"]) != "45432"
        or port["target"] != 5432
    ):
    ...
    url = (
        ...
        f"{quote(values['POSTGRES_PASSWORD'], safe='')}@127.0.0.1:45432/"
        f"{quote(values['POSTGRES_DB'], safe='')}"
    )
    environment = {
        **os.environ,
        "DATABASE_URL": url,
        "CHECKOUT_COPILOT_DATABASE_URL": url,
        "CHECKOUT_TEST_DATABASE_URL": url,
        "CHECKOUT_COPILOT_EXECUTION_MODE": "scripted",
