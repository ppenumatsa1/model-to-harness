...
import pytest
from checkout_recovery_maf.config import Settings, checkout_env_file
...
def isolated_settings(monkeypatch, tmp_path):
    for key in os.environ:
        if key.upper().startswith("CHECKOUT_RECOVERY_") or key.upper() == (
...
def test_only_editable_lane_is_discovered(tmp_path, monkeypatch):
    lane = tmp_path / "harness/checkout-recovery/maf"
    module = lane / "backend/src/checkout_recovery_maf/config.py"
    module.parent.mkdir(parents=True)
    (lane / "pyproject.toml").write_text("", encoding="utf-8")
    monkeypatch.chdir(tmp_path)
    assert checkout_env_file(module) == lane / ".env"
    for other in (
        tmp_path / "site-packages/checkout_recovery_maf/config.py",
        lane / "infra/foundry-hosted/agent/checkout_recovery_maf/config.py",
        tmp_path / "other/maf/backend/src/checkout_recovery_maf/config.py",
    ):
...
def test_settings_precedence_and_telemetry_alias(tmp_path, monkeypatch):
    ...
    env_file.write_text(
        ...
        "CHECKOUT_RECOVERY_DATABASE_URL=synthetic-dotenv-database\n"
    ...
    monkeypatch.setenv("CHECKOUT_RECOVERY_API_PORT", "18020")
...
def test_none_disables_dotenv_but_preserves_process_settings(tmp_path, monkeypatch):
    env_file = tmp_path / "selected.env"
    env_file.write_text("CHECKOUT_RECOVERY_API_PORT=8001\n", encoding="utf-8")
...
def test_cwd_and_other_dotenv_files_are_not_searched(tmp_path, monkeypatch):
    ...
    selected.write_text("CHECKOUT_RECOVERY_API_PORT=18020\n", encoding="utf-8")
    cwd = tmp_path / "unrelated"
    cwd.mkdir()
    for name in (".env", ".env.local", ".env.production", ".env.compose"):
        (cwd / name).write_text("CHECKOUT_RECOVERY_API_PORT=9000\n", encoding="utf-8")
...
@pytest.mark.parametrize("host", ["", "http://localhost", "localhost:8000", "bad/name", "bad host"])
...
def test_frontend_origin_override(monkeypatch):
    monkeypatch.setenv("CHECKOUT_RECOVERY_FRONTEND_ORIGIN", "http://127.0.0.1:15175/")
...
@pytest.mark.parametrize(
    ...
    [
        ...
        {"database_url": "synthetic-db", "api_token": "synthetic-token", "execution_mode": "maf"},
    ],
)
...
def test_launcher_passes_one_selected_settings_snapshot(monkeypatch):
    from checkout_recovery_maf import main
...
def test_module_launcher_selects_settings_once(monkeypatch):
    ...
    monkeypatch.setattr(main, "create_app", factory)
    monkeypatch.setattr("uvicorn.run", run)
    runpy.run_module("checkout_recovery_maf", run_name="__main__")
