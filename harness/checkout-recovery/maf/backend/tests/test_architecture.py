...
SOURCE = ROOT / "backend/src/checkout_recovery_maf"
...
with (
    ...
    import checkout_recovery_maf
    import checkout_recovery_maf.bootstrap as bootstrap
    import checkout_recovery_maf.main as main
    import checkout_recovery_maf.projections
    assert not hasattr(bootstrap, 'app')
    assert not hasattr(main, 'app')
"""
    env = {
        key: value for key, value in os.environ.items() if not key.startswith("CHECKOUT_RECOVERY_")
    }
...
def test_application_and_runtime_never_import_api_and_projections_are_pure():
    forbidden = {
        "application": ("checkout_recovery_maf.api", "fastapi"),
        "projections": (
            "checkout_recovery_maf.api",
            "checkout_recovery_maf.infrastructure",
            "checkout_recovery_maf.maf",
            "fastapi",
            "psycopg",
        ),
    }
    ...
    for name in ("bootstrap.py", "config.py", "__init__.py"):
        ...
        assert "checkout_recovery_maf.api" not in text
...
class BlockApi(importlib.abc.MetaPathFinder):
    def find_spec(self, fullname, path, target=None):
        if fullname.startswith('checkout_recovery_maf.api'):
...
with (
    patch('psycopg_pool.ConnectionPool', side_effect=AssertionError('pool at import')),
    patch('checkout_recovery_maf.bootstrap.configure_api_telemetry',
          side_effect=AssertionError('Hosted API telemetry')),
    patch('checkout_recovery_maf.bootstrap.create_runtime',
...
assert not any(name.startswith('checkout_recovery_maf.api') for name in sys.modules)
"""
    result = subprocess.run(
