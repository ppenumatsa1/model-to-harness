...
import pytest
from agent_framework import FunctionTool, observability
from checkout_recovery_maf.infrastructure import telemetry
from checkout_recovery_maf.infrastructure.telemetry import SafeExporter, operation
...
def test_exporter_redacts_every_content_surface_without_dropping_spans(monkeypatch):
    ...
    assert span.attributes == {"checkout.component": "maf", "checkout.correlation": CORRELATION}
    ...
    for item in collector.spans:
        ...
        assert item.resource.attributes == {"service.name": "checkout-recovery-maf-api"}
...
def test_native_maf_tool_parents_survive_across_export_batches(monkeypatch, batch_size):
            ...
            with operation("start", "case-id"), operation("model"):
                with tracer.start_as_current_span(
                    ...
                    for tool_name in ("read_order", "read_payment", "read_inventory"):
                        tool = FunctionTool(name=tool_name, description=SECRET, func=lambda: None)
                        attributes = observability.get_function_span_attributes(tool, SECRET)
                        with observability.get_function_span(attributes):
