...
describe("server-only development configuration", () => {
  it("defaults to loopback API 8000 and UI 5173 without dotenv", () => {
  ...
  it("discovers only the editable lane, never the cwd or an installed parent", () => {
    ...
    const lane = join(root, "checkout-recovery/maf");
  ...
  it("reads only the selected file and lets process overrides win", () => {
    ...
    writeFileSync(envFile, [
      "CHECKOUT_RECOVERY_API_HOST=localhost",
      "CHECKOUT_RECOVERY_API_PORT=8001",
      'CHECKOUT_RECOVERY_FRONTEND_ORIGIN="http://localhost:5174"',
      "APPLICATIONINSIGHTS_CONNECTION_STRING=synthetic-secret",
      "CHECKOUT_RECOVERY_API_TOKEN=synthetic-secret",
      "VITE_SECRET=synthetic-secret"
    ].join("\n"));
    writeFileSync(join(root, ".env.local"), "CHECKOUT_RECOVERY_API_PORT=9000");
    ...
    expect(developmentSettings({}, join(root, "missing.env")).port).toBe(5173);
  });

  it.each(["0", "65536", "1.5", "abc", "", "0x50"])("rejects invalid API port %s", (value) => {
    expect(() => developmentSettings({ CHECKOUT_RECOVERY_API_PORT: value }, null)).toThrow("API_PORT");
  });

  it.each(["https://localhost:5173", "http://example.com", "http://0.0.0.0:5173",
    "http://localhost:5173/path", "http://user:secret@localhost:5173",
    "http://localhost:5173?secret", "http://localhost:5173#secret",
    "http://localhost:0", "http://localhost:65536", "http://localhost:5173\\evil"])(
    "rejects unsafe development origins without echoing values", (value) => {
      expect(() => developmentSettings({ CHECKOUT_RECOVERY_FRONTEND_ORIGIN: value }, null))
        .toThrow("must be a loopback HTTP origin");
    }
  );

  it.each(["http://localhost", "localhost:8000", "localhost/path", "user@localhost", ""])(
    "rejects malformed proxy hosts", (value) => {
      expect(() => developmentSettings({ CHECKOUT_RECOVERY_API_HOST: value }, null)).toThrow("API_HOST");
    }
  );

  it.each([["0.0.0.0", "127.0.0.1"], ["::", "[::1]"], ["::1", "[::1]"]])(
    "maps wildcard/IPv6 API host %s to a usable proxy target", (host, target) => {
      expect(developmentSettings({ CHECKOUT_RECOVERY_API_HOST: host }, null).apiTarget)
        .toBe(`http://${target}:8000`);
    }
  );
